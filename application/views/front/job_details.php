<?php $this->load->view('front/include/header');?>
<!--inner heading start-->
<div class="inner-heading">
  <div class="container">
    <h3>ELAN HAQQINDA ƏTRAFLI OXUYUN</h3>
  </div>
</div>
<!--inner heading end--> 

<!--Inner Content start-->
<div class="inner-content listing detail">
  <div class="container"> 
    
    <!--Detail page start-->
    <div class="inner-wrap">
      <div class="row">
        <div class="col-md-8">
          <div class="listWrpService jobdetail">
            <div class="row">
              <div class="col-md-3 col-sm-3 col-xs-3">
                <div class="listImg"><img src="<?=base_url('assets/front/');?>images/jobdetail.jpg" alt=""></div>
              </div>
              <div class="col-md-9 col-sm-9 col-xs-9">
                <h3>Marketing Graphic Design / 2D Artist</h3>
                <p>Design Communication Studio</p>
                <ul class="featureInfo">
                  <li><i class="fa fa-map-marker" aria-hidden="true"></i> Atlanta, GA</li>
                  <li><i class="fa fa-calendar" aria-hidden="true"></i> Dec 30, 2015 - Feb 20, 2016 </li>
                </ul>
                <div class="time-btn">Part Time</div>
                <div class="click-btn"><a href="#">Apply Now</a></div>
              </div>
            </div>
            <h2>Discription</h2>
            <p> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed bibendum, eros a luctus tristique, dui lorem accumsan est, id gravida mauris libero et dolor. Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Vivamus orci eros, cursus ac sollicitudin vitae, sagittis vel arcu. Aenean porta sed purus in fringilla. Curabitur convallis augue eget urna euismod convallis. Sed porttitor risus vitae justo gravida semper. Cras placerat dolor quis erat consequat sagittis vitae vitae lacus. Suspendisse ultricies molestie mauris, sit amet bibendum purus pharetra sed. Pellentesque porttitor malesuada tincidunt. Aliquam ac sem elit. <br/>
              <br/>
              Integer ullamcorper egestas diam sit amet posuere. Donec elementum pharetra dapibus. Vivamus tincidunt nisl semper eleifend lacinia. Integer nec nisl porttitor libero pretium consequat iaculis a eros. Donec eu risus ultrices metus imperdiet efficitur in et mi. Proin bibendum nunc ac felis faucibus pulvinar. Phasellus in commodo ipsum. Donec lacinia sit amet libero non viverra. Maecenas non sodales tellus.</p>
            <h2>Education Requirements</h2>
            <p>Proin aliquet sit amet est nec semper. Aenean vitae neque odio. Sed non orci quis libero scelerisque interdum. Nulla facilisi. Suspendisse sem erat, ornare vitae rhoncus eu, vulputate vitae velit. Curabitur fringilla iaculis enim, vitae aliquam neque.</p>
            <ul class="featureLinks">
              <li>Nullam neque arcu</li>
              <li>Opencart listed Partne</li>
              <li>Maecenas ultricies lobortis tristique</li>
              <li>Pellentesque dapibus metus dolor</li>
            </ul>
            <h2>Specification</h2>
            <p>Working knowledge in Web Design having the following Skills in these tools and technologies</p>
            <ul class="featureLinks">
              <li>HTML5</li>
              <li>CSS3</li>
              <li>Java Script J-Query</li>
              <li>Adobe Photoshop</li>
              <li>Adobe Illustrator</li>
              <li>CorelDraw</li>
              <li>Flash Dreamweaver</li>
            </ul>
            <h2>Rewards and Benefits</h2>
            <ul class="featureLinks benefits">
              <li> Incentive Bonus</li>
              <li>check_circle</li>
              <li>Medical</li>
              <li>Transport</li>
              <li>Sports</li>
            </ul>
            <div class="clearfix"></div>
          </div>
        </div>
        <div class="col-md-4">
          <div class="sidebarWrp listWrpService jobdetail">
            <h3>About Company</h3>
            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque ut velit eget leo mollis gravida non quis mauris. Maecenas vestibulum, nunc congue aliquam tincidunt, orci metus cursus magna, a vulputate nisl arcu sed elit. Maecenas convallis auctor sodales. Donec non blandit neque.</p>
            <div class="companyInfo">Industry</div>
            <p>Design Communication Studio</p>
            <div class="companyInfo">No. of Employees</div>
            <p>99</p>
            <div class="companyInfo">Location</div>
            <p>New York</p>
            <div class="map">
              <iframe src="<?=base_url('assets/front/');?>https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d387193.30593401584!2d-74.25986539089548!3d40.69714941954754!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x89c24fa5d33f083b%3A0xc80b8f06e177fe62!2sNew+York%2C+NY!5e0!3m2!1sen!2sus!4v1506615745397" width="100" height="250" frameborder="0" style="border:0" allowfullscreen></iframe>
            </div>
            <form>
              <div class="contactWrp">
                <h3>Contact Now</h3>
                <div class="input-wrap">
                  <input type="text" name="" placeholder="Name:" class="form-control">
                </div>
                <div class="input-wrap">
                  <input type="text" name="" placeholder="Name:" class="form-control">
                </div>
                <div class="input-wrap">
                  <input type="text" name="" placeholder="Your Email" class="form-control">
                </div>
                <div class="input-wrap">
                  <textarea class="form-control" placeholder="Type Your Message here.."></textarea>
                </div>
                <div class="contact-btn">
                  <button class="sub" type="submit" value="submit" name="submitted"> Submit Now</button>
                </div>
              </div>
            </form>
          </div>
        </div>
      </div>
    </div>
    <!--Detail page end--> 
    
  </div>
</div>
<!--Inner Content end--> 
<?php $this->load->view('front/include/footer');?>